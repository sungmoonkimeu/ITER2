...
if __name__ == '__main__':
    ...
    f = open('EWOFS_fig4_saved0.txt', 'w')
    ...
    abs_error = zeros([len(Temp_LF), len(V_I)])
    rel_error = zeros([len(Temp_LF), len(V_I)])

    #start_time = time.time()
    f = open('EWOFS_fig4_saved0.txt', 'a')
    ...
    for mm in range(len(Temp_LF)):
        ...
        print(" %s degC was calcualated, (%s / %s)" % (Temp_LF[mm], mm,len(Temp_LF)))
