...
for mm in range(len(LB)):
    ...
    fig, axis = S[0].draw_poincare(kind='both')
    ...
    V_J = V_out.T.reshape(len(V_I),2).T
    print(V_J)

    S.from_Jones(E.from_matrix(V_J))
    S.draw_poincare(axis, kind='both', angle_view=[0.5, -1], figsize=(8, 6), remove_depol=True)

    print(S)
    #axis, fig = S[0].draw_poincare(kind='both')
